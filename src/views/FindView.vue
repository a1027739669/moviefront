<template>
  <div class="find-view" @dblclick="button_restore()">
    ...
    <div class="movies-result">
      ...
      <div class="single-movie" @click="goToMovie(movie.movieid)" @mouseenter="change_movieclass(movie)" @mouseleave="re_movieclass()">
...
export default {
  ...
  methods: {
    loadMore () {
      ...
      this.loading = true
      this.limit += 6
      if (this.current_index_keywords === '剧情') {
        this.get_movies()
      } else {
        var keywords = this.current_index_keywords
        var sort = this.current_index_sort
        var year = this.relase_year
        var region = this.origin_place
        movieApi.searchMovie(keywords, sort, year, region, this.limit, this.offset)
      .then(function (res) {
